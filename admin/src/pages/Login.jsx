...
const Login = () => {
   ...
   const onSubmitHandler= async(e)=>{
     e.preventDefault(); //not load the webpage
     try{
        if(state==='Admin')
        {
    ...
    <form onSubmit={onSubmitHandler} className='min-h-[80vh] flex items-center'>
      <div className='flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] sm:min-w-96 border rounded-xl text-[#5E5E5E] text-sm shadow-lg'>
        ...
        <div className='w-full'>
            <p>Email</p>
            <input onChange={(e)=>setEmail(e.target.value)} value={email} className='border border-[#DADADAD] rounded w-full p-2 mt-1' type="email" required></input>
        </div>
        <div className='w-full'>
            <p>Password</p>
            <input onChange={(e)=>setPassword(e.target.value)} value={password} className='border border-[#DADADAD] rounded w-full p-2 mt-1' type="password" required></input>
        </div>
